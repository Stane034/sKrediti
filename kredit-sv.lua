...
ESX.RegisterServerCallback('Kredit:fetchStatus', function(source, cb)
    ...
    local status = MySQL.Sync.fetchAll('SELECT * FROM users WHERE identifier = @id', {
        ['@id'] = identifier
    })
    cb(status[1])
end)
...
AddEventHandler('Kredit:vracajNovac', function(kolicina)
    ...
    local updatovanostanje = trenutnostanje - tonumber(kolicina)
        if igrac.getMoney() >= tonumber(kolicina) then
            ...
            MySQL.Sync.execute('UPDATE users SET kredit = @updatovanostanje WHERE identifier = @id ', {
                ['@id'] = identifier, 
...
function VratiDane()
    local krediti = MySQL.Sync.fetchAll('SELECT * FROM users')
    for i = 1, #krediti, 1 do 
        if krediti[i].danikredita < 7 and krediti[i].kredit > 0 then
            MySQL.Sync.execute('UPDATE users SET danikredita = danikredita + 1 WHERE identifier = @id', {
  ...
  for i = 1, #svinaSrw, 1 do
   ...
   for i = 1, #krediti, 1 do 
    if xPlayer.identifier == krediti[i].identifier then
        ...
        xPlayer.removeAccountMoney('bank', Cfg.RataKredita)
        xPlayer.showNotification('Oduzeto vam je ' .. Cfg.RataKredita .. '$ kao rata kredita.')
        MySQL.Sync.execute('UPDATE users SET kredit = kredit - @rata WHERE identifier = @id', { 
            ['@rata'] = Cfg.RataKredita,
            ['@id'] = xPlayer.identifier,
        })
    end 
   end
...
function VratiOfflineBackEnd(krediti)
    for i = 1, #krediti, 1 do
       local statusi = json.decode(krediti[i].accounts)
        ...
        if krediti[i].kredit == 0 then
            MySQL.Sync.execute('UPDATE users SET danikredita = 0 WHERE identifier = @id', { 
                ['@id'] = krediti[i].identifier,
            })
        end
       
        local updatovaniStatusi = { 
            bank = statusi.bank - Cfg.RataKredita,
            money = statusi.money,
            black_money = statusi.black_money
        }

        MySQL.Sync.execute('UPDATE users SET accounts = @acc WHERE identifier = @id', { 
